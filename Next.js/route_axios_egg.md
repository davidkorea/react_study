前端路由 VS 后端路由
1. next的路由，用于不同url显示不同页面，而页面中的不同数据内容，实在不同页面函数下面的getInitialProps中做数据请求得到的
2. egg路由，根据不同url，找到对应的控制器，执行控制器里面的方法函数中的逻辑，获取/查询数据后，再返回给前端的getInitialProps

所以可以看到，网页前端的url时查询参数`?id=1`的方式传递参数，而后端的api时动态路由`list/id/12`的方式传递参数。实际上两者传递参数没有关系
- 前端页面根据参数，到达新页面，并发传递的参数`?id=1`给到新页面的getInitialProps中取请求后端api
- 而后端api是根据getInitialProps中设置的请求url `list/id/1`去做响应
- 参数id是同一个数值，但是前后端的传递方式不同，也互不影响，直到参数可以正确传递即可
